...
type DepartmentQuery struct {
	...
	withInstructors *InstructorQuery
...
// QueryInstructors chains the current query on the "instructors" edge.
func (dq *DepartmentQuery) QueryInstructors() *InstructorQuery {
	query := (&InstructorClient{config: dq.config}).Query()
...
func (dq *DepartmentQuery) QueryCourses() *CourseQuery {
	query := (&CourseClient{config: dq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := dq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (dq *DepartmentQuery) QueryStudents() *StudentQuery {
...
func (dq *DepartmentQuery) Clone() *DepartmentQuery {
	...
	return &DepartmentQuery{
		...
		withInstructors: dq.withInstructors.Clone(),
		withCourses:     dq.withCourses.Clone(),
...
// WithInstructors tells the query-builder to eager-load the nodes that are connected to
// the "instructors" edge. The optional arguments are used to configure the query builder of the edge.
func (dq *DepartmentQuery) WithInstructors(opts ...func(*InstructorQuery)) *DepartmentQuery {
	query := (&InstructorClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	dq.withInstructors = query
	return dq
}
...
func (dq *DepartmentQuery) WithCourses(opts ...func(*CourseQuery)) *DepartmentQuery {
	query := (&CourseClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	dq.withCourses = query
	return dq
}
...
func (dq *DepartmentQuery) WithStudents(opts ...func(*StudentQuery)) *DepartmentQuery {
	query := (&StudentClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (dq *DepartmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Department, error) {
	...
	if query := dq.withInstructors; query != nil {
		if err := dq.loadInstructors(ctx, query, nodes,
			func(n *Department) { n.Edges.Instructors = []*Instructor{} },
			func(n *Department, e *Instructor) { n.Edges.Instructors = append(n.Edges.Instructors, e) }); err != nil {
			return nil, err
		}
	}
	if query := dq.withCourses; query != nil {
		if err := dq.loadCourses(ctx, query, nodes,
			func(n *Department) { n.Edges.Courses = []*Course{} },
			func(n *Department, e *Course) { n.Edges.Courses = append(n.Edges.Courses, e) }); err != nil {
			return nil, err
		}
	}
	if query := dq.withStudents; query != nil {
		if err := dq.loadStudents(ctx, query, nodes,
			func(n *Department) { n.Edges.Students = []*Student{} },
			func(n *Department, e *Student) { n.Edges.Students = append(n.Edges.Students, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (dq *DepartmentQuery) loadInstructors(ctx context.Context, query *InstructorQuery, nodes []*Department, init func(*Department), assign func(*Department, *Instructor)) error {
	...
	query.withFKs = true
	query.Where(predicate.Instructor(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(department.InstructorsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.department_instructors
		if fk == nil {
			return fmt.Errorf(`foreign-key "department_instructors" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "department_instructors" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (dq *DepartmentQuery) loadCourses(ctx context.Context, query *CourseQuery, nodes []*Department, init func(*Department), assign func(*Department, *Course)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Department)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Course(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(department.CoursesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.department_courses
		if fk == nil {
			return fmt.Errorf(`foreign-key "department_courses" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "department_courses" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (dq *DepartmentQuery) loadStudents(ctx context.Context, query *StudentQuery, nodes []*Department, init func(*Department), assign func(*Department, *Student)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Department)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Student(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(department.StudentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.department_students
		if fk == nil {
			return fmt.Errorf(`foreign-key "department_students" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "department_students" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
